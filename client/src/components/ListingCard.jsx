...
const ListingCard = ({
  ...
  const handleCardClick = (e) => {
    // Prevent the event from propagating when clicking inside the card
    e.stopPropagation();
    navigate(`/listing/${listingId}`);
  };

  const handleWishlistClick = (e) => {
    // Prevent the event from propagating when clicking the wishlist button
    e.stopPropagation();
    patchWishList();
  };
  ...
  return (
    <div 
      ...
      className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6 bg-white shadow-lg rounded-3xl ring-1 ring-gray-300 cursor-pointer group transition hover:shadow-xl"
    >
      {/* Image Carousel */}
